...
# ✅ 修复注册 DGLHeteroGraph 的补丁：必须放在 import dgl 后面
import sys
try:
    dummy_graph = dgl.heterograph({('a', 'r', 'b'): ([0], [1])})
    hetero_graph_type = type(dummy_graph)
    sys.modules['dgl.heterograph'].DGLHeteroGraph = hetero_graph_type
except Exception as e:
    raise ImportError("Failed to infer DGL graph type for patching: %s" % str(e))


def safe_dill_load(file_obj):
    import dgl
    try:
        from dgl.heterograph import DGLHeteroGraph
        globals()['DGLHeteroGraph'] = DGLHeteroGraph
    except ImportError:
        dummy = dgl.heterograph({('a', 'r', 'b'): ([0], [1])})
        globals()['DGLHeteroGraph'] = type(dummy)
    import dill
    ...
    def _prepare(self):
        ...
        for molecule in self.data:
            ...
            for src, dst in edge_list:
                g.add_edges(src.item(), dst.item())  # 加边
...
class MoleculeDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name):
        ...
        with open(data_dir + name + '.pkl', "rb") as f:
            f = safe_dill_load(f)  # (train_list, val_list, test_list, num_atom_type, num_bond_type)

            self.train = f[0]
            self.val = f[1]
            self.test = f[2]
            self.num_atom_type = f[3]  # 28
            self.num_bond_type = f[4]  # 4
    ...
    def collate(self, samples):  # mini-batch 拼接
        # The input samples is a list of pairs (graph, label).
        graphs, labels = map(list, zip(*samples))

        labels = torch.tensor([float(lbl) for lbl in labels]).unsqueeze(1)
    ...
    def _make_full_graph(self):  # 添加全连接图结构
    ...
    def _add_laplacian_positional_encodings(self, pos_enc_dim):  # Laplacian 编码
    ...
    def _add_wl_positional_encodings(self):  # WL 编码
