...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def propagate_attention(self, g, mask=None):
        ...
        '''if mask in ['random', 'attention']:
            score = g.edata['score'].squeeze(-1)  # [E, H]
            E, H = score.shape
            num_mask = int(E * 0.2)

            if mask == 'random':
                torch.manual_seed(42)
                idx = torch.randperm(E, device=score.device)[:num_mask]
                # 对所有 head 的相同边统一遮盖
                score[idx] = 0.0

            elif mask == 'attention':
                for h in range(H):
                    head_score = score[:, h]  # [E]
                    idx = torch.topk(head_score, k=num_mask, largest=True).indices
                    score[idx, h] = 0.0

            g.edata['score'] = score.unsqueeze(-1)  # [E, H, 1]'''
    ...
    def forward(self, g, h, e):
        ...
        self.propagate_attention(g, mask='attention')
