            ...
            for smiles, sub_dic in tqdm(drugs_summary):
                ...
                with open(mol_path, 'rb') as f:
                    ...
                    for conformer_dict in conformer_list[:self.n_conf]:
                        ...
                        # ✅ 推荐写法（避免注册为 PyG 的 tensor 属性）
                        data._smiles = smiles  # 注意下划线！

                        # data.__dict__['_smiles'] = smiles  # ✅ 添加隐藏字段
                        # if 'smiles' in data.keys:  # 避免 PyG 注册字段错误
                        #     del data['_smiles']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_folder', default='/nfs_home/xiaofeng/zhen/GraphMVP_xf/datasets/SLURM_TMPDIR/', type=str)
